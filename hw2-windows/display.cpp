...
void display() 
{
  ...
  // glPolygonMode(GL_FRONT_AND_BACK, GL_LINE); // TEMPORARY
  ...
  if (numused) {
	...
	GLfloat currentLightDirection[4];
	GLfloat reworkedLightDirection[4];

	for (int i = 0; i < numused; i++) {
		for (int j = 0; j < 4; j++) {
			currentLightDirection[j] = lightposn[i * 4 + j];
		}
		transformvec(currentLightDirection, reworkedLightDirection);
		for (int j = 0; j < 4; j++) {
			lightransf[i * 4 + j] = reworkedLightDirection[j];
		}
	}

	glUniform1f(numusedcol, numused);
  ...
  transf = modelview * tr * sc ; // We are creating the result matrix.
